...
class KERNALMODULE {
	...
	button( parent, id, text, menuOptions ) {
		...
		div1.style.borderTop = "#ccc 1px solid";
		div1.style.borderBottom = "#ccc 1px solid";
		div1.style.borderLeft = "#777 1px solid";
		div1.style.borderRight = "#777 1px solid";
	...
	getClickLinkFunc() {
		...
		return function(){
			...
			if( this.dataset.action.startsWith("@")  ) {
				__this.basic[ this.dataset.action.substr(1) ]( this.dataset.data );
			}
			else {
				__this.handlers[ "handler_" + this.dataset.action ]( this.dataset.data );
			}
		}
	}
